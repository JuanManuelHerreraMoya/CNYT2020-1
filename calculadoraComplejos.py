...
def moduloSinRaiz(num1):
    resu = ((num1[0]**2)+(num1[1]**2))
    return resu
def inversa(num1):
...
##sE AÑADIO LA PROBABILDAD DE ENCONTRAR UNA POSICION EN PARTICULAR
def probabilidadPosicion(vec1,pos):
    if pos < len(vec1) :
        resu = 0
        for i in range(len(vec1)):
            resu = resu + moduloSinRaiz(vec1[i])
        probabilidad = moduloSinRaiz(vec1[pos])/resu
        print(probabilidad)
    return(probabilidad)

def probabilidadTransitarPrimerVectorAlSegundo(mat1,vec1):
##def main():
...
##    probabilidadPosicion(vector1,9)
##main()
